...
public class OrderTimeoutScheduler {
    ...
    private void cancelOrderAndReleaseStock(OrderInfo order) {
        ...
        if ("LOCKED".equals(lockStatus)) {
            // 已锁定 → 归还库存 + 回退销量（通过 Feign 调用商品服务）
            List<OrderItem> items = orderItemService.list(
                    new LambdaQueryWrapper<OrderItem>().eq(OrderItem::getOrderId, orderId));
            StockReleaseDTO dto = new StockReleaseDTO();
            java.util.List<StockReleaseDTO.Item> releaseItems = new java.util.ArrayList<>();
            for (OrderItem item : items) {
                StockReleaseDTO.Item ri = new StockReleaseDTO.Item();
                ri.setSkuId(item.getSkuId());
                ri.setSpuId(item.getSpuId());
                ri.setQuantity(item.getQuantity());
                releaseItems.add(ri);
            }
            dto.setItems(releaseItems);
            productFeignClient.releaseStock(dto);
