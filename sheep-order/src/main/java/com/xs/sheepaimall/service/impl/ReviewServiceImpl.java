...
public class ReviewServiceImpl extends ServiceImpl<ProductReviewMapper, ProductReview> implements ReviewService {
    ...
    public Page<ReviewVO> pageBySpu(Long spuId, int pageNum, int pageSize) {
        Spu spu = productFeignClient.getSpuById(spuId);
        if (spu == null) {
    ...
    public Page<ReviewVO> pageByMerchant(int pageNum, int pageSize, Integer status, Integer reviewStatus, String keyword) {
        Long userId = UserContext.getUserId();
        Merchant merchant = merchantFeignClient.getMerchantByUserId(userId);
        if (merchant == null) {
    ...
    public Page<ReviewVO> pageByMerchantPublic(Long merchantId, int pageNum, int pageSize, Integer rating) {
        // 查该店铺所有 SPU ID
        List<Long> spuIds = productFeignClient.listSpuIdsByMerchant(merchantId);
        if (spuIds.isEmpty()) {
            return new Page<>(pageNum, pageSize);
        }
    ...
    public Page<ReviewVO> pageAllReview(int pageNum, int pageSize, String keyword,
        ...
        if (StrUtil.isNotBlank(keyword)) {
            List<Long> matchedSpuIds = productFeignClient.listSpuIdsByKeyword(keyword);
            List<Long> matchedUserIds = authFeignClient.listUserIdsByKeyword(keyword);
    ...
    private ReviewVO toReviewVO(ProductReview review) {
        ...
        Spu spu = productFeignClient.getSpuById(review.getSpuId());
        if (spu != null) vo.setSpuName(spu.getName());
        Sku sku = productFeignClient.getSkuById(review.getSkuId());
        if (sku != null) vo.setSkuName(sku.getSkuName());
        SysUser user = authFeignClient.getUserById(review.getUserId());
        if (user != null) {
    ...
    private Page<ReviewVO> toReviewVOPage(Page<ProductReview> page) {
        ...
        Map<Long, String> spuNameMap = productFeignClient.listSpuByIds((List<Long>) spuIds).stream()
        ...
        Map<Long, SysUser> userMap = authFeignClient.listUsersByIds((List<Long>) userIds).stream()
