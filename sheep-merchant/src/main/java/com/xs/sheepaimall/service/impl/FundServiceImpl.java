...
public class FundServiceImpl extends ServiceImpl<FundFlowMapper, FundFlow> implements FundService {
    ...
    private CommissionConfigVO toCommissionConfigVO(CommissionConfig config) {
        ...
        Category cat = productFeignClient.getCategoryById(config.getCategoryId());
    ...
    private OrderCommissionLogVO toOrderCommissionLogVO(OrderCommissionLog log) {
        ...
        // SPU 名称
        Spu spu = productFeignClient.getSpuById(log.getSpuId());
        if (spu != null) vo.setSpuName(spu.getName());
        // 分类名称
        Category cat = productFeignClient.getCategoryById(log.getCategoryId());
        if (cat != null) vo.setCategoryName(cat.getName());
        // 店铺名称
    ...
    public void settleOrderCommission(Long orderId) {
        ...
        // 查询订单明细
        List<OrderItem> items = orderFeignClient.getOrderItemsByOrderId(orderId);
        if (items.isEmpty()) {
        ...
        for (OrderItem item : items) {
            ...
            Spu spu = productFeignClient.getSpuById(item.getSpuId());
